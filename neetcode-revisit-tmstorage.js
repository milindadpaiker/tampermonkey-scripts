// ==UserScript==
// @name         NeetCode Revisit Tagger - Nonlocal
...
(function () {
    ...
    // Utility functions for Tampermonkey persistent storage
    const getRevisitList = async () => {
        const list = await GM_getValue('revisitList', '[]'); // Default to an empty array if no value exists
        return JSON.parse(list);
    };

    const saveRevisitList = async (list) => {
        await GM_setValue('revisitList', JSON.stringify(list));
    };
    ...
    function addRevisitButton() {
        ...
        const updateRevisitButtonState = async () => {
            ...
            if (revisitList.includes(problemUrl)) {
                ...
                revisitButton.onclick = async() => {
                    ...
                    await saveRevisitList(updatedList);
                ...
                revisitButton.onclick = async() => {
                    ...
                    await saveRevisitList(revisitList);
